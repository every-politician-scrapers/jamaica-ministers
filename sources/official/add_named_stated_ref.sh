...
item=$(qsv search -i $1 wikidata.csv | qsv select $WIKIDATA_ITEM | qsv behead)
statementid=$(qsv search -i $1 wikidata.csv | qsv select $WIKIDATA_PSID | qsv behead)
...
name=$(qsv search -i $1 scraped.csv | qsv select $SCRAPED_NAME | qsv behead)
claims=$(qsv search -i $1 scraped.csv | qsv select $SCRAPED_NAME,$SCRAPED_POSN | qsv behead | qsv fmt --out-delimiter " ")
